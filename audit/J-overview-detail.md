...
web 端已在大厅补入 `GeneralsOverviewPage` 武将一览入口，覆盖包列表、搜索、禁包/禁将编辑、反选、计数 footer 和点击武将看技能弹窗；但卡牌一览、武将筛选、武将池、战绩列表、统计页仍无独立页面。
...
原版这些页面的入口在 `Fk/Pages/Lobby/LobbyElements`（武将一览/卡牌一览按钮）与对局准备/结算流程（武将池、战绩）；web 目前只实现了大厅「武将一览」入口，其余入口仍缺。
...
- web : apps/web/src/components/GeneralsOverviewPage.tsx + apps/web/src/pages/LobbyPage.tsx
...
- web 行为: 大厅 header 增加「武将一览」入口；弹窗页使用 CatalogVm 读取 Lua 武将/包 metadata，左侧包列表 + 顶部搜索/禁包/禁将/反选/退出按钮 + GeneralCard 网格 + footer 计数；点击武将打开现有 GeneralDetailModal 技能弹窗。
- 差异: 已有可用页和核心禁将编辑入口；仍缺原版 Filter 按钮/长按重置、vanish/appear 动画、0.6×0.8 GeneralDetailPage 完整详情页和部分像素级样式。
- 修复: 已修复并验证 (2026-06-13: GeneralsOverviewPage 接入大厅入口并用 CatalogVm 渲染真实包/武将数据；pnpm web test/typecheck/build、WSL web-asio 构建、gateway+asio E2E 均通过；仍按简化还原跟踪完整筛选/动画/详情页缺口)

### J2 GeneralsOverview::大包小包列表(modList/pkgList)
- 状态: 简化还原
- 原版: GeneralsOverview.qml:26-115 (modList ListView / pkgList ListView)
- web : apps/web/src/components/GeneralsOverviewPage.tsx + apps/web/src/stores/serverManifestStore.ts + apps/web/src/stores/index.ts
...
- web : 左侧按 module/package 双列显示大包/小包，点击切包过滤网格；SetServerSettings 读取 serverHiddenPacks 并过滤隐藏包；禁包状态在编辑模式内同步显示。
- 差异: 数据来源和过滤语义已接近原版；高亮/锁图标/动画不是逐像素照搬。
- 修复: 已修复并验证 (2026-06-13: 接入 hiddenPacks manifest 字段和双列包列表；serverManifestStore/disableSchemes 相关测试通过)
...
- web : apps/web/src/components/GeneralsOverviewPage.tsx
- 原版行为: TextField 输入名字，回车或🔍按钮触发；word.text 非空时 Ltk.searchAllGenerals(全局搜)，否则 Ltk.searchGenerals(当前小包, "")
- web : 搜索框输入后按武将 id/翻译名在已加载 CatalogVm 数据中筛选；非空搜索跨当前可见武将集合生效，空搜索回到当前包。
- 差异: 搜索结果由前端 metadata 过滤实现，未暴露原版 Ltk.searchGenerals/searchAllGenerals 桥接，也未还原筛选页回传条件。
- 修复: 已修复并验证 (2026-06-13: 搜索功能随武将一览页落地；pnpm web test/typecheck/build 通过)
...
- web : apps/web/src/components/GeneralsOverviewPage.tsx + apps/web/src/stores/disableSchemesStore.ts + apps/web/src/components/CreateRoomDialog.tsx
...
- web : 新增 disableSchemes localStorage store 和 curScheme 概念；武将一览页支持禁包、禁将、反选，CreateRoom 将 banPkg/normalPkg/banCardPkg 按原版 CreateRoom.qml 语义转换为 disabledGenerals/disabledPack。
- 差异: stat 状态机以 React UI state 实现；禁将遮罩/文案非逐像素照搬；卡包 banCardPkg 尚无独立 RoomPackageSettings UI。
- 修复: 已修复并验证 (2026-06-13: disableSchemes.test 覆盖 banPkg/normalPkg 转换、禁包/禁将切换和非 lunarltk 空 payload；真实 asio/gateway E2E 建房通过)
...
- web : footer 显示当前过滤后的武将数量，并随切包/搜索更新。
- 差异: 字体和 footer 位置不逐像素等同原版。
- 修复: 已修复并验证 (2026-06-13: footer 计数随 GeneralsOverviewPage 落地；pnpm web test/typecheck/build 通过)
...
|------|------|
| 完全还原 | 0 |
| 简化还原 | 8 (J1, J2, J3, J4, J5, J6, J7, J9) |
| 还原错误 | 0 |
| 未还原 | 18 (J8, J10–J26) |
| 合计 | 26 |

## 未还原序号索引
J8, J10, J11, J12, J13, J14, J15, J16, J17, J18, J19, J20, J21, J22, J23, J24, J25, J26

## 还原错误序号索引
（无）

## 简化还原序号索引
J1（武将一览整页：已有入口/包列表/网格/禁将编辑，缺完整筛选/动画/详情页）、J2（大包小包列表：已有双列包列表和 hiddenPacks，缺锁图标/动画等像素级细节）、J3（搜索：前端 metadata 过滤，非 Ltk search 桥接）、J4（禁将/禁包：核心方案和 CreateRoom 转换已接通，缺卡包设置页和部分视觉）、J5（计数 footer：已有动态计数，字体/位置简化）、J6（武将详情整页：仅技能页、无标签框架/信息栏/按钮）、J7（立绘未放大、布局不同）、J9（技能列表：颜色不同、无链接展开、缺 headnote/companions/endnote）
...
- 原版入口位置：武将一览/卡牌一览/武将池在 Lobby 与对局准备流程；战绩/统计在 Replay 模块。web `pages/LobbyPage.tsx` 目前只有武将一览入口，卡牌一览/武将池/战绩/统计仍无对应入口。
