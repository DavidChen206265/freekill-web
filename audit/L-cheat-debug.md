...
- 差异: 简化——用「按钮+搜索框+扩展包/势力筛选」替代原版「按包分层 StackView」(分层浏览未还原,但筛选+搜索功能等价或更强);入口用显式按钮而非右键候选(更易发现,不与 web 已有的右键/长按看技能 IG-6 冲突)。
- 修复: 已修复并验证 (新增 VM 桥 searchGenerals[返回 name+extension+kingdom,支持 pack 过滤]/generalPacks/getSetting + GeneralBox 自由选将按钮 + FreeAssignOverlay[搜索框+扩展包 select+势力 select+滚动网格];真 VM 探针验证桥[searchGenerals 28将/带 ext+kingdom、generalPacks、pack 过滤、Translate(caocao)=曹操];typecheck/build/161 测试全绿。2026-06-12,由未还原→简化还原。**实测反馈三修(2026-06-12)**:① 武将名显示原始拼音→overlay 在结果变化时 registerTranslations+mergeGenerals[根因:这些名不在 popup active.generals,vmStore 不会自动注册];② popup 超出屏幕→panel 固定高+grid flex:1/minHeight:0/overflowY 滚动;③ 无筛选→加扩展包+势力下拉。同类 i18n 隐患已写入 CLAUDE.md 实现纪律 §7。)
