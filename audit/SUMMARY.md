...
| **合计** | | **135** | **142** | **0** | **182** | **459** |

> 上表为修复后的当前值。初始审计合计错误 10/完全 165。已处理全部 10 条:M3/N2/H6/E9/B40/B41/D11/C29→完全,I8→简化(核心 bug 修复、弹幕分流待 I9),N20→简化(IG-5b 有意简化,与 H20–H24 一致)。**还原错误清零(10→0)**;C3 战绩面板随 C29 由未还原→简化；N1-2 余项 M14/M15 已补齐；N1-3 已补 P9/P10/P11 上报入口并使 C19/C20 降为简化；N2 已补 playing/faceturned/saveme 状态视觉；N1-4 已补核心手牌拖拽/超级拖拽/双击和手牌信息；2026-06-13 禁将系统与大厅武将一览切片 B-E 已使 B30、J1-J5 降为简化还原；G2/G4 detailed 选项框已补描述卡片并升为完全还原。
...
- **P 协议上报剩余 3 项**：prelight 预亮（F7）、changeskin 换肤、ChangeRoom 改房——asio 支持但 web 前端无入口；投降/托管/房主踢人已于 2026-06-13 补齐并验证，托管补强为点击后立即本地清理当前询问 UI、全局显示/隐藏托管遮罩与「退出托管」、进入/退出用 pending 乐观态等待 NetStateChanged 校准、GameOver 清理托管渲染态并阻断其它交互。
- **C19 游戏内菜单 overlay 剩余项**：已有最小局内菜单 + 投降/托管；设置、一览、聊天抽屉、Esc/缩放效果仍未完整还原。
- **E14/E15/E17 手牌出牌交互剩余简化项**：核心拖拽/重排、拖到目标/OK、双击使用已于 2026-06-13 补齐；已补强卡牌动画释放后拖拽实时跟随指针,以及超级拖拽移动中选牌/目标、释放时重读 OK 后确认；仍缺完整 ControlSetting/Config 驱动的 `enableSuperDrag`/`doubleClickUse`/`autoTarget` 开关。
...
- **J 总览/详情/战绩页族（18 条未还原 + 8 条简化）**：武将一览入口、包列表、搜索、禁包/禁将和计数已简化还原；仍缺完整 GeneralDetailPage、卡牌一览、武将筛选、武将池、战绩列表、统计页。
- **B 筛选 + 建房子系统**：禁将方案（B30）已简化还原并接入建房 disabled payload；FilterRoom 整套（B4/B17）、Lua 动态设置 UI（B28）、卡包设置（B29）仍未还原。
...
- **G 请求弹窗骨架**：21 个 Box + 5 个 request_type 全部至少有可用 port，核心规则桥（poxiFilter/Feasible/Prompt、chooseGeneralFilter、choiceFilter、cardFitPattern、防作弊 shuffle）全部存在且签名匹配；detailed choice/choices 已补描述卡片，utility 共享 CustomDialog 已白名单移植，包专用 QML/MiniGame 仍显式 unsupported（0 未还原/0 错误）。
