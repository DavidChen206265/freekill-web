...
- **P 协议上报剩余 3 项**：prelight 预亮（F7）、changeskin 换肤、ChangeRoom 改房——asio 支持但 web 前端无入口；投降/托管/房主踢人已于 2026-06-13 补齐并验证。
- **C19 游戏内菜单 overlay 剩余项**：已有最小局内菜单 + 投降/托管；设置、一览、聊天抽屉、Esc/缩放效果仍未完整还原。
- **E14/E15/E17 手牌出牌交互剩余简化项**：核心拖拽/重排、拖到目标/OK、双击使用已于 2026-06-13 补齐；仍缺完整 ControlSetting/Config 驱动的 `enableSuperDrag`/`doubleClickUse`/`autoTarget` 开关和逐帧进入/离开 Photo 切换细节。
