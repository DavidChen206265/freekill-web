...
- web : 无（`apps/web/src/diag/notifyCommands.ts:58` SetBanner∈KNOWN_DEFERRED）
- 原版行为: 全局顶部横幅区，复用 MarkArea.setMark 同前缀规则展示全局标记（如身份场、特殊模式提示），紫色半透明底框
- web 行为: SetBanner 命令被标 deferred（INFO 级），不渲染任何 banner 区域
- 差异: 整个全局 banner 区缺失（GameSummary 类 banner 仅在 GameOver 另行读取 clientVm.ts:386，非此 UI 区）
...
- web : 无（`notifyCommands.ts:59` UpdateMarkArea∈KNOWN_DEFERRED）
- 原版行为: 服务器可整体隐藏/显示某玩家的两个标记区（markArea+picMarkArea visible）
- web 行为: 命令 deferred，标记区恒显（不响应 visible 切换）
- 差异: 标记区显隐控制未实现
...
| 状态 | 数量 | 序号 |
|------|------|------|
| 完全还原 | 7 | M2, M8, M11, M12, M13, M16, M17 |
| 简化还原 | 5 | M1, M4, M7, M9, M10 |
| 还原错误 | 1 | M3 |
| 未还原 | 4 | M5, M6, M14, M15 |
| 合计 | 17 | |
...
- M14 SetBanner 全局顶部横幅区
- M15 UpdateMarkArea 标记区 visible 显隐控制

## 还原错误索引
- M3 `@$`/`@&` 牌堆标记：原版显示**张数**，web 显示**逐项翻译拼接内容**（clientVm.ts:150-169 缺 `@$`/`@&` 专属计数分支）
